...
namespace Assets.Minecraft
{
    class Chunk
    {
        ...
        {
            if (relx < 0)
                return world.GetChunk(Pos.x - 1, Pos.y).GetBlock(relx + Settings.ChunkSize.x, rely, relz);
            if (relx >= Settings.ChunkSize.x)
                return world.GetChunk(Pos.x + 1, Pos.y).GetBlock(relx - Settings.ChunkSize.x, rely, relz);
            if (rely < 0 || rely >= Settings.ChunkSize.y)
                return BlockType.Air;
            if (relz < 0)
                return world.GetChunk(Pos.x, Pos.y - 1).GetBlock(relx, rely, relz + Settings.ChunkSize.z);
            if (relz >= Settings.ChunkSize.z)
                return world.GetChunk(Pos.x, Pos.y + 1).GetBlock(relx, rely, relz - Settings.ChunkSize.z);
        ...
        {
            if (relx < 0)
                world.GetChunk(Pos.x - 1, Pos.y).SetBlock(relx + Settings.ChunkSize.x, rely, relz, type);
            else if (relx >= Settings.ChunkSize.x)
                world.GetChunk(Pos.x + 1, Pos.y).SetBlock(relx - Settings.ChunkSize.x, rely, relz, type);
            else if (relz < 0)
                world.GetChunk(Pos.x, Pos.y - 1).SetBlock(relx, rely, relz + Settings.ChunkSize.z, type);
            else if (relz >= Settings.ChunkSize.z)
                world.GetChunk(Pos.x, Pos.y + 1).SetBlock(relx, rely, relz - Settings.ChunkSize.z, type);
            else
            {
