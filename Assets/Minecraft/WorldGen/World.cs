...
namespace Assets.Minecraft
{
    class World : MonoBehaviour
    {
        ...
        {
            int cx = 100, cz = 100;
            if (!chunkManager.ChunkExists(cx, cz))
                GenerateChunk(cx, cz);
            Player.position = new Vector3(7.5f + cx * Settings.ChunkSize.x, GetChunk(cx, cz).HeightMap[7, 7] + 3, 7.5f + cz * Settings.ChunkSize.z);
        }
        ...
        public BlockType GetBlock(int x, int y, int z)
        {
            ...
            if (!chunkManager.ChunkExists(c.x, c.y))
                return BlockType.Air;
